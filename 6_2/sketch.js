...
function distanceWrapped(x1, y1, x2, y2) {
    ...
    // Take wrapping around borders into account
...
function getRandomUnitVector() {
  ...
  return createVector(cos(angle), sin(angle)); // Convert the angle to a 2D unit vector
}
...
function fitness(swarm) {
    // Initialize sum of normalized velocities
    let sumX = 0;
    let sumY = 0;

    // Calculate sum of normalized velocities
    ...
    // Calculate the magnitude of the sum of normalized velocities
    let sumMagnitude = Math.sqrt(sumX * sumX + sumY * sumY);

    // Divide by the number of boids to get the average magnitude
    let order = sumMagnitude / swarm.length;

    return order;
}
// Adjust the setup function to pass the speed parameter when creating new particles
simulations = []
function setup_simulations()
{
    for (let i = 0; i < simulation_count; i++) 
    {
        ...
        for (let i of Array(swarm_size)){
            randomSeed(i);
            sim.scene.swarm.push(new Particle(speed,sim.scene,sim.parameters)); // Pass the speed parameter
        }
...
function draw() {
    ...
    for (let j = 0; j < simulations.length; j++) 
    {
        ...
        //             simulations[j].parameters.cohesion.toString() + "," +
        //             simulations[j].parameters.alignment.toString() + "," +
        //             simulations[j].parameters.separation.toString() + "," +
        //             fitness(simulations[j].scene.swarm).toString() + "\n");
        simulations[j].print();
    }
...
class Particle {
    ...
    constructor(speed, scene, parameters){ // Add speed as a parameter
        this.scene = scene;
    ...
    step(){
        ...
        // Normalize the direction vector and scale it to the constant speed
